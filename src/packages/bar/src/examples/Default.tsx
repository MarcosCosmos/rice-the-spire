...
const BoundMenuBar = ({ children }: { children: ReactNode }) => {
    ...
    const bindingModeClasses = hasMode
      ? zebar?.glazewm?.bindingModes.map(mode => 'menubar--binding-mode-' + mode).join('')
      : 'menubar--no-binding-mode';
    return (
        <MenuBar className={bindingModeClasses}>
            {children}
        </MenuBar>
    );
...
const Default = () => {
  const [spireConfig, setSpireConfig] = useState<SpireConfig>(defaultSpireConfig);
  useEffect(() => {    
